...
class Weapon(Item):
    def __init__(self, item_level, player_name, weapon_name = None):
        ...
        if weapon_name:
           ...
           # weapon_index = min(item_level - 1, 2) #wapen level-uped als speler level-uped 
            if player_name == "Finn" :
    ...
    def set_damage(self):
        if self.weapon_type == "Knife🔪":
            ...
            self.min_damage =  self.item_level * 2 + 10
            self.max_damage = self.item_level * 3 + 10

        elif self.weapon_type == "Gunter🐧":
            self.min_damage =  self.item_level * 3 + 10
            self.max_damage = self.item_level * 4 + 10
